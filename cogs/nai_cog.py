...
class NAI(commands.Cog):
    ...
    @app_commands.command(name="fix_first_gen", description="Fixes the first generation timestamp for a user (Owner only)")
    @app_commands.describe(
        user="The user whose first generation timestamp to fix"
    )
    async def fix_first_gen(self, interaction: discord.Interaction, user: discord.User):
        """Fixes the first generation timestamp for a user"""
        logger.info(f"COMMAND 'FIX_FIRST_GEN' USED BY: {interaction.user} ({interaction.user.id}) for user {user.id}")
        ...
        user_id_to_fix = user.id
        history_file_path = settings.GLOBAL_STATS_DIR / "nai_history.json"
        user_stats_file_path = settings.USER_STATS_DIR / "nai_user_stats.json"

        try:
            # Read history data
            # NOTE: This might fail if the history file is too large, as experienced before.
            # We need to handle this limitation.
            try:
                with open(history_file_path, 'r') as f:
                    history_data = json.load(f)
            except Exception as e:
                 await interaction.followup.send(f"❌ Error reading history file: {str(e)}. The file might be too large to process automatically. Manual intervention is required.", ephemeral=True)
                 logger.error(f"Error reading history file for fix_first_gen: {str(e)}")
                 return

            # Find the earliest timestamp for the user
            user_history = [
                entry for entry in history_data
                if entry.get("user_id") == user_id_to_fix
            ]

            if not user_history:
                await interaction.followup.send(f"No generation history found for user {user.mention}.", ephemeral=True)
                return

            # Sort by timestamp and get the earliest
            sorted_user_history = sorted(user_history, key=lambda x: datetime.fromisoformat(x.get("timestamp")))
            earliest_timestamp = sorted_user_history[0].get("timestamp")

            if not earliest_timestamp:
                 await interaction.followup.send(f"Could not find a valid earliest timestamp for user {user.mention}.", ephemeral=True)
                 return

            # Read user stats data
            try:
                with open(user_stats_file_path, 'r') as f:
                    user_stats_data = json.load(f)
            except Exception as e:
                 await interaction.followup.send(f"❌ Error reading user stats file: {str(e)}", ephemeral=True)
                 logger.error(f"Error reading user stats file for fix_first_gen: {str(e)}")
                 return

            # Update the user's first_generation timestamp
            user_id_str = str(user_id_to_fix)
            if user_id_str in user_stats_data:
                user_stats_data[user_id_str]["first_generation"] = earliest_timestamp

                # Save the updated user stats data
                try:
                    temp_user_stats_file = user_stats_file_path.with_suffix('.tmp')
                    with open(temp_user_stats_file, 'w') as f:
                        json.dump(user_stats_data, f, indent=2)
                    temp_user_stats_file.replace(user_stats_file_path)

                    await interaction.followup.send(f"✅ Successfully updated the first generation timestamp for user {user.mention} to `{earliest_timestamp[:10]}`.", ephemeral=True)
                    logger.info(f"Successfully fixed first generation timestamp for user {user_id_to_fix}")

                except Exception as e:
                    await interaction.followup.send(f"❌ Error saving updated user stats file: {str(e)}", ephemeral=True)
                    logger.error(f"Error saving user stats file after fix_first_gen: {str(e)}")
            else:
                await interaction.followup.send(f"User {user.mention} not found in user statistics.", ephemeral=True)

        except Exception as e:
            logger.error(f"Unexpected error in fix_first_gen command: {str(e)}")
