...
class GameCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.stats_file = Path("database/wordle_stats.json")
        self.history_file = Path("database/wordle_history.json")
    ...
    def _load_word_list(self) -> tuple[set, set]:
        ...
        try:
            # Load answer words
            with open("database/wordle-answers-alphabetical.txt", "r") as f:
                answers = {line.strip() for line in f if line.strip()}
            
            # Load allowed guesses
            with open("database/wordle-allowed-guesses.txt", "r") as f:
    ...
    @app_commands.describe(word="Type a 5-letter word and use autocomplete to guess the word.")
